...
class VoteRepository
{
    ...
    {
        $this->connec = DbConnexion::connection();
    }

    private function recordExists(int $webtoon)
    {
        $query = $this->connec->prepare("SELECT * FROM webtoon WHERE id = :web");
    ...
    private function vote(int $webtoon, $user, int $vote)
    {
        ...
        $query = $this->connec->prepare("SELECT id, vote FROM votes WHERE webtoon = :web AND user = :user");
        $query->bindValue(':web', $webtoon);
        ...
        if ($vote_row) {
            if ($vote_row->vote == $vote) {
                $this->connec->query('DELETE FROM votes WHERE id = ' . $vote_row->id);
                return false;
            }
            $this->former_vote = $vote_row;
            $query = $this->connec->prepare("UPDATE votes SET vote = ? WHERE id = {$vote_row->id}")->execute([$vote]);
            return true;
        }
        $query = $this->connec->prepare("INSERT INTO votes(webtoon, user, vote) VALUES(:web, :user, :vot)");
        $query->bindValue(':web', $webtoon);
        $query->bindValue(':user', $user);
    ...
    {
        $query = $this->connec->prepare("SELECT COUNT(id) as count, vote FROM votes WHERE webtoon = :web GROUP BY vote");
        $query->bindValue(':web', $webtoon);
        $query->execute();
    ...
    {
        $query = $this->connec->prepare("SELECT * FROM votes WHERE user = :user AND webtoon = :web");
        $query->bindValue(':web', $webtoon);
        $query->bindValue(':user', $user);
        $query->execute();
        $query->setFetchMode(\PDO::FETCH_OBJ);
